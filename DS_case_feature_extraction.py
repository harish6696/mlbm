...
from validator_nomask import GridValidator
from custom_double_shear_dataset import CustomDataset, DataTransform
...
@hydra.main(version_base="1.3", config_path="./conf_ds", config_name="config.yaml") # conf_ds
def main(cfg: DictConfig):   
    ...
    validator = GridValidator(out_dir=ckpt_args["path"] + "/validators", loss_fun=MSELoss(), num_output_channels=cfg.arch.decoder.out_features, num_input_channels=cfg.arch.fno.in_channels, case_name=cfg.data.case_name ,mean_info=cfg.data.normalization_mean, std_info=cfg.data.normalization_std) 
