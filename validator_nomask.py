...
class GridValidator:
    ...
    def compare(
        ...
        for input_channel in range(self.num_input_channels):
            ...
            im[input_channel][0] = ax[input_channel][0].imshow(invar_plot, cmap=colormap) # deleted self.obs_mask
        ...
        for channel in range(self.num_output_channels): 
            ...
            im[channel][1] = ax[channel][1].imshow(target_plot, cmap=colormap)
            fig.colorbar(im[channel][1], ax=ax[channel][1], location="bottom", fraction=0.046, pad=0.04)
            ax[channel][1].set_title(self.headers[1])

            # im[channel][2] = ax[channel][2].imshow(prediction_plot, cmap=colormap)
            im[channel][2] = ax[channel][2].imshow(prediction_plot, cmap=colormap)
            ...
            im[channel][3] = ax[channel][3].imshow(np.abs(target_plot - prediction_plot), cmap=colormap)
            ...
            im[channel][4] = ax[channel][4].imshow(np.clip(((prediction_plot - target_plot) / target_plot), a_min=-1.0, a_max=1.0), cmap=colormap)
